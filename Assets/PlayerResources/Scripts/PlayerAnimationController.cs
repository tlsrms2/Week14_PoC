...
public class PlayerAnimationController : MonoBehaviour
{
    ...
    {
        if (!TryGetCursorWorldPosition(out Vector2 cursorWorldPosition))
        {
            ApplyFacing(currentFacing);
            return;
        }

        Vector2 toCursor = cursorWorldPosition - (Vector2)transform.position;
        ...
        UpdateVisualRootFlip(toCursor.x);
        UpdateLeftArmAim(cursorWorldPosition);

        float angleFromDown = Vector2.Angle(Vector2.down, toCursor);
        VisualFacing nextFacing = GetFacingFromAngle(angleFromDown);

        ApplyFacing(nextFacing);
    }
    ...
    private bool TryGetCursorWorldPosition(out Vector2 worldPosition)
    {
        ...
        screenPosition.z = Mathf.Abs(playerCamera.transform.position.z - transform.position.z);
        worldPosition = playerCamera.ScreenToWorldPoint(screenPosition);
        return true;
    }

    private void UpdateVisualRootFlip(float cursorOffsetX)
    {
        if (visualRoot == null || Mathf.Abs(cursorOffsetX) <= flipDeadZone)
        {
            return;
        }
        ...
        scale.x = cursorOffsetX < 0f ? -xMagnitude : xMagnitude;
    ...
    {
        if (playerController == null || !playerController.WasShootPressedThisFrame || leftArmAnimator == null)
        {
            return;
        }
    ...
    private void UpdateInterceptAnimation()
    {
        if (playerController == null || !playerController.WasInterceptPressedThisFrame)
        {
            return;
        }
    ...
    private void UpdateLeftArmAim(Vector2 cursorWorldPosition)
    {
        if (leftArm == null)
        {
            return;
        ...
        Vector2 cursorLocalPosition = aimSpace.InverseTransformPoint(cursorWorldPosition);
        Vector2 armLocalPosition = leftArm.localPosition;
        Vector2 localDirection = cursorLocalPosition - armLocalPosition;
