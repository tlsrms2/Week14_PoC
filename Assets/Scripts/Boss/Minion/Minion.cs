...
namespace Week14.Enemy
{
    ...
    public sealed class Minion : MonoBehaviour
    {
        ...
        private IMinionMovementPathIndicatorOwner movementPathIndicatorOwner;
            ...
            MinionGraphProjectileFireSpec aimSpec)
        {
            ...
            RotateToDirection(direction);
            BeginMovementPathIndicator(
                transform.position,
                (Vector2)transform.position + direction.normalized * Mathf.Max(0f, dashSpeed) * duration);
            if (!IsExecutionPaused)
            {
            ...
            bool useStartPlayerPosition)
        {
            ...
            float travelled = 0f;
            bool lockedToPattern = false;
            BeginOrbitMovementPathIndicator(startCenter, radius);
        ...
        private void SetPatternPosition(Vector2 target, ref bool lockedToPattern, float moveSpeed, bool showMovementPathIndicator = true)
        {
            if (lockedToPattern)
            {
                SetPlayerCollisionIgnored(false);
                if (showMovementPathIndicator)
                {
                    EndMovementPathIndicator();
                }
            ...
            if (showMovementPathIndicator)
            {
                BeginMovementPathIndicator(transform.position, target);
            }
        ...
        private void BeginMovementPathIndicator(Vector2 start, Vector2 end)
        {
            movementPathIndicatorPoints.Clear();
            movementPathIndicatorPoints.Add(start);
            movementPathIndicatorPoints.Add(end);
            BeginMovementPathIndicator(movementPathIndicatorPoints, false);
        }

        private void BeginOrbitMovementPathIndicator(Vector2 center, float radius)
        {
            movementPathIndicatorPoints.Clear();
            float safeRadius = Mathf.Max(0.1f, radius);
            ...
            {
                movementPathIndicatorPoints.Add(center + AngleToDirection(360f * i / CircleSegments) * safeRadius);
            }

            BeginMovementPathIndicator(movementPathIndicatorPoints, true);
        }

        private void BeginMovementPathIndicator(IReadOnlyList<Vector2> points, bool loop)
        {
            movementPathIndicatorOwner ??= Owner as IMinionMovementPathIndicatorOwner;
            movementPathIndicatorOwner?.BeginMinionMovementPathIndicator(this, points, loop);
        }

        private void TickMovementPathIndicator(Vector2 current)
        {
            movementPathIndicatorOwner?.TickMinionMovementPathIndicator(this, current);
        }

        private void EndMovementPathIndicator()
        {
            movementPathIndicatorOwner?.EndMinionMovementPathIndicator(this);
            movementPathIndicatorOwner = null;
        }
