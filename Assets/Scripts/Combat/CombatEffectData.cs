...
namespace Week14.Combat
{
    ...
    public sealed class CombatEffectData : ScriptableObject
    {
        ...
        [Tooltip("플레이어 일반 공격 탄과 총구 화염에 사용하는 기본 색입니다.")]
        ...
        [Tooltip("패링 성공 시 링 주변에 생성되는 글리터 수입니다.")]
        ...
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 전방 스파크 색입니다.")]
        [SerializeField] private Color weakAttackImpactSparkColor = new(0.82f, 0.88f, 1f, 1f);
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 반대 방향 스파크 색입니다.")]
        [SerializeField] private Color weakAttackImpactBackSparkColor = new(0.55f, 0.66f, 0.85f, 1f);
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 화염 입자 색입니다.")]
        [SerializeField] private Color weakAttackImpactFlameColor = new(0.58f, 0.68f, 0.9f, 1f);
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 충격 링 색입니다.")]
        [SerializeField] private Color weakAttackImpactRingColor = new(0.82f, 0.88f, 1f, 0.5f);
        [Tooltip("유효 거리 밖에서 플레이어 공격이 적에게 맞을 때 생성되는 약한 전방 스파크 수입니다.")]
        [SerializeField, Min(0)] private int weakAttackImpactSparkCount = 4;
        [Tooltip("유효 거리 밖에서 플레이어 공격이 적에게 맞을 때 반대 방향으로 튀는 약한 스파크 수입니다.")]
        [SerializeField, Min(0)] private int weakAttackImpactBackSparkCount = 1;
        [Tooltip("유효 거리 밖에서 플레이어 공격이 적에게 맞을 때 생성되는 약한 화염 입자 수입니다.")]
        [SerializeField, Min(0)] private int weakAttackImpactFlameCount = 0;
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 전체 크기 배율입니다.")]
        [SerializeField, Min(0f)] private float weakAttackImpactEffectScale = 0.18f;
        [Tooltip("유효 거리 밖 약한 피격 이펙트의 투명도 배율입니다.")]
        [SerializeField, Range(0f, 1f)] private float weakAttackImpactAlpha = 0.35f;
        [Tooltip("플레이어가 적 공격에 맞을 때 생성되는 전방 스파크 수입니다.")]
        [SerializeField, Min(0)] private int playerHitSparkCount = 12;
        [Tooltip("플레이어가 적 공격에 맞을 때 반대 방향으로 튀는 스파크 수입니다.")]
        [SerializeField, Min(0)] private int playerHitBackSparkCount = 5;
        [Tooltip("플레이어가 적 공격에 맞을 때 생성되는 화염 입자 수입니다.")]
        ...
        public int WeakAttackImpactSparkCount => weakAttackImpactSparkCount;
        public int WeakAttackImpactBackSparkCount => weakAttackImpactBackSparkCount;
        public int WeakAttackImpactFlameCount => weakAttackImpactFlameCount;
        public float WeakAttackImpactEffectScale => weakAttackImpactEffectScale > 0f ? weakAttackImpactEffectScale : 0.18f;
        public float WeakAttackImpactAlpha => Mathf.Clamp01(weakAttackImpactAlpha);
        public Color WeakAttackImpactSparkColor => ApplyAlpha(weakAttackImpactSparkColor, WeakAttackImpactAlpha);
        public Color WeakAttackImpactBackSparkColor => ApplyAlpha(weakAttackImpactBackSparkColor, WeakAttackImpactAlpha);
        public Color WeakAttackImpactFlameColor => ApplyAlpha(weakAttackImpactFlameColor, WeakAttackImpactAlpha);
        public Color WeakAttackImpactRingColor => ApplyAlpha(weakAttackImpactRingColor, WeakAttackImpactAlpha);
        ...
        private static Color ApplyAlpha(Color color, float alphaMultiplier)
        {
