...
[CustomEditor(typeof(Arsonist))]
[CanEditMultipleObjects]
public sealed class ArsonistEditor : Editor
{
    ...
    private static readonly HashSet<string> ArsonistFields = new()
    {
        "bossGraph",
        "graphProjectiles",
        "oilTrailDuration",
    ...
    private bool showArsonistSettings = true;
    ...
    private void DrawBossGraphSection()
    {
        ...
        if (graph != null)
        {
            EditorGUILayout.HelpBox("현재 할당된 Boss Graph를 사용합니다.", MessageType.Info);
        }
        else if (!hasMultipleValues)
        {
            EditorGUILayout.HelpBox("Boss Graph가 비어 있으면 Arsonist는 패턴을 실행하지 않습니다.", MessageType.Warning);
        }
    ...
    private void DrawArsonistSettingsSection()
    {
        showArsonistSettings = EditorGUILayout.Foldout(showArsonistSettings, "방화범 설정", true);
        if (!showArsonistSettings)
        {
            return;
        }

        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        DrawProperty("oilTrailDuration");
        DrawProperty("oilTrailRadius");
        DrawProperty("ignitedOilDuration");
        DrawProperty("oilConnectionRadius");
        DrawProperty("oilColor");
        EditorGUILayout.Space(4f);
        DrawProperty("oilSoakedDuration");
        DrawProperty("oilTrailInterval");
        DrawProperty("oilTrailMinDistance");
        EditorGUILayout.Space(4f);
        DrawProperty("fireDamage");
        DrawProperty("fireDamageInterval");
        DrawProperty("fireColor");
        EditorGUILayout.Space(4f);
        DrawProperty("burnDamage");
        DrawProperty("burnTickInterval");
        DrawProperty("burnDuration");
        EditorGUILayout.EndVertical();
    }
    ...
    {
        return targets.Length == 1 && target is Arsonist boss ? boss.transform : null;
    }
    ...
    {
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        DrawPropertiesBoxContents(title, propertyNames);
        EditorGUILayout.EndVertical();
    }

    private void DrawPropertiesBoxContents(string title, params string[] propertyNames)
    {
    ...
    private void DrawBaseProperties()
    {
        ...
        while (property.NextVisible(enterChildren))
        {
            ...
            if (IsPropertyNamed(property, "m_Script")
                || ContainsPropertyName(ArsonistFields, property)
