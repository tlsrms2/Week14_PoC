...
namespace Week14.UI
{
    // 패시브 스킬 로스터의 스킬칸 하나입니다. 슬롯과 무관하게 하나만 존재하는 공유 목록의 아이템입니다.
    ...
    {
        private const PassiveSkillSlot ActiveSlot = PassiveSkillSlot.Passive1;
        ...
        {
            if (subscribedToSkillEquipped && PassiveSkillLoadoutManager.Instance != null)
            {
                PassiveSkillLoadoutManager.Instance.SkillEquipped -= HandleSkillEquipped;
            }
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            bool isEquipped = PassiveSkillLoadoutManager.Instance.GetEquippedSkill(ActiveSlot) == skill;

            if (eventData.button == PointerEventData.InputButton.Left)
            {
                if (state == LoadoutSkillLockState.Locked)
                {
                    if (GameSaveManager.PurchasePassiveSkill(skill.SkillId, skill.Price))
                    {
                        RefreshVisualState();
                        LoadoutSelectedSkillPanel.Instance?.Show(skill);
                    }
                ...
                {
                    PassiveSkillLoadoutManager.Instance.EquipSkill(ActiveSlot, skill.SkillId);
                    LoadoutSelectedSkillPanel.Instance?.Show(skill);
                }
            }
            else if (eventData.button == PointerEventData.InputButton.Right)
            {
                if (state == LoadoutSkillLockState.Purchased)
                {
                    if (isEquipped)
                    {
                        PassiveSkillLoadoutManager.Instance.UnequipSkill(ActiveSlot);
                        LoadoutSelectedSkillPanel.Instance?.Show(skill);
                    }
                    else if (PassiveSkillLoadoutManager.Instance.RefundSkill(skill.SkillId))
                    {
                        RefreshVisualState();
                        LoadoutSelectedSkillPanel.Instance?.Show(skill);
                    }
        ...
        private void HandleSkillEquipped(PassiveSkillSlot slot, BasePassiveSkillSO _)
        {
            if (slot == ActiveSlot)
            {
                RefreshEquippedIndicator();
        ...
        {
            if (skill == null || !GameSaveManager.IsPassiveSkillUnlocked(skill.SkillId))
            {
                return LoadoutSkillLockState.NotUnlocked;
            }

            return GameSaveManager.IsPassiveSkillPurchased(skill.SkillId)
